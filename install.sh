...
install_amneziawg() {
    echo -e "${green}Installing AmneziaWG runtime...${plain}"
    ...
    install_ndppd() {
        case "${release}" in
            ...
            fedora | amzn | virtuozzo | rhel | almalinux | rocky | ol | centos)
                dnf install -y -q ndppd 2> /dev/null || yum install -y ndppd 2> /dev/null || true
                ;;
            arch | manjaro | parch)
                pacman -Sy --noconfirm ndppd 2> /dev/null || true
                ;;
            alpine)
                apk add ndppd 2> /dev/null || true
                ;;
        esac
    }
    ...
    case "${release}" in
        ubuntu | debian | armbian)
            ...
            if [[ "${release}" == "ubuntu" ]]; then
                add-apt-repository -y ppa:amnezia/ppa 2> /dev/null && apt-get update -q && apt-get install -y amneziawg || true
            else
                apt-key adv --keyserver keyserver.ubuntu.com --recv-keys 57290828 2> /dev/null || true
                if ! grep -q "ppa.launchpadcontent.net/amnezia/ppa" /etc/apt/sources.list 2> /dev/null; then
                ...
                apt-get update -q && apt-get install -y amneziawg || true
            fi
            ;;
        fedora | amzn | virtuozzo | rhel | almalinux | rocky | ol | centos)
            dnf install -y -q wireguard-tools 2> /dev/null || yum install -y wireguard-tools 2> /dev/null || true
            echo -e "${yellow}Install amneziawg-tools manually for full AmneziaWG support on this distro.${plain}"
            ;;
        arch | manjaro | parch)
        ...
        alpine)
            ...
            echo -e "${yellow}Install amneziawg-tools manually for full AmneziaWG support on Alpine.${plain}"
            ;;
        *)
            echo -e "${yellow}Unknown OS. Install amneziawg-tools manually: https://github.com/amnezia-vpn/amneziawg-linux-kernel-module${plain}"
            ;;
    esac
    ...
    if command -v awg > /dev/null 2>&1 && command -v awg-quick > /dev/null 2>&1; then
        echo -e "${green}AmneziaWG installed: $(awg --version 2> /dev/null || echo awg)${plain}"
    else
        echo -e "${yellow}Warning: awg/awg-quick not found. The panel can save AmneziaWG inbounds, but local tunnels will not start until AmneziaWG tools are installed.${plain}"
    fi
}
...
install_x-ui() {
    ...
    if [ $# == 0 ]; then
        ...
        url="https://github.com/spiridonburgeranov/3iax-uilo/releases/download/${tag_version}/x-ui-linux-$(arch).tar.gz"
    ...
    curl -fLRo "${xui_script_temp}" https://raw.githubusercontent.com/spiridonburgeranov/3iax-uilo/main/x-ui.sh
    if [[ $? -ne 0 ]]; then
        rm -f "${xui_script_temp}"
    ...
    if [[ $release == "alpine" ]]; then
        ...
        rm -f "${xui_rc_temp}"
        curl -fLRo "${xui_rc_temp}" https://raw.githubusercontent.com/spiridonburgeranov/3iax-uilo/main/x-ui.rc
        if [[ $? -ne 0 ]]; then
            rm -f "${xui_rc_temp}"
        ...
        if [ "$service_installed" = false ]; then
            ...
            case "${release}" in
                ubuntu | debian | armbian)
                    service_unit_url="https://raw.githubusercontent.com/spiridonburgeranov/3iax-uilo/main/x-ui.service.debian"
                    ;;
                arch | manjaro | parch)
                    service_unit_url="https://raw.githubusercontent.com/spiridonburgeranov/3iax-uilo/main/x-ui.service.arch"
                    ;;
                *)
                    service_unit_url="https://raw.githubusercontent.com/spiridonburgeranov/3iax-uilo/main/x-ui.service.rhel"
                    ;;
            esac

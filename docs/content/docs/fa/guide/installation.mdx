...
bash <(curl -Ls https://raw.githubusercontent.com/spiridonburgeranov/3iax-uilo/main/install.sh) v3.4.0
```
...
docker run -d \
  ...
  --name 3x-ui \
  ghcr.io/spiridonburgeranov/3iax-uilo:latest
```
...
[صفحهٔ releases](https://github.com/spiridonburgeranov/3iax-uilo/releases) دانلود کنید، آن را
...
```bash
XUI_NONINTERACTIVE=1 bash <(curl -Ls https://raw.githubusercontent.com/spiridonburgeranov/3iax-uilo/main/install.sh)
```

دایرکتوری [`deploy/`](https://github.com/spiridonburgeranov/3iax-uilo/tree/main/deploy) در مخزن

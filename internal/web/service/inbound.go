...
type InboundOption struct {
	...
	AwgJc          int    `json:"awgJc,omitempty"`
	AwgJmin        int    `json:"awgJmin,omitempty"`
...
func (s *InboundService) GetInboundOptions(userId int) ([]InboundOption, error) {
	...
	for _, r := range rows {
		wgHints := inboundWireguardHints(r.Protocol, r.Settings)
		...
		out = append(out, InboundOption{
			...
			WgPublicKey:       wgHints.PublicKey,
			WgMtu:             wgHints.MTU,
			WgDns:             wgHints.DNS,
			AwgJc:             wgHints.Jc,
			AwgJmin:           wgHints.Jmin,
...
type inboundWireguardHint struct {
	PublicKey string
	MTU       int
	DNS       string
	Jc        int
...
func inboundWireguardHints(protocol string, settings string) inboundWireguardHint {
	if (protocol != string(model.WireGuard) && protocol != string(model.AmneziaWG)) || strings.TrimSpace(settings) == "" {
...
// Only vmess, vless, trojan, shadowsocks, hysteria, wireguard, amneziawg, and tunnel
...
func (s *InboundService) SetInboundEnable(id int, enable bool) (bool, error) {
	...
	if inbound.NodeID == nil && inbound.Protocol == model.AmneziaWG {
		...
		if err := db.Transaction(func(tx *gorm.DB) error {
			...
			if enable {
				_ = (&AwgInboundService{}).Disable(inbound)
			}
			return false, err
		}
		inbound.Enable = enable
		if !enable {
			if err := (&AwgInboundService{}).Disable(inbound); err != nil {
				return false, err
			}
		}
		return false, nil
...
func (s *InboundService) UpdateInbound(inbound *model.Inbound) (*model.Inbound, bool, error) {
	...
	txErr := runSerializedTx(func(tx *gorm.DB) error {
		...
		isLocalAWGBefore := oldInbound.NodeID == nil && oldProtocol == model.AmneziaWG
		isLocalAWGAfter := oldInbound.NodeID == nil && oldInbound.Protocol == model.AmneziaWG
		if isLocalAWGBefore || isLocalAWGAfter {
			if isLocalAWGBefore && !isLocalAWGAfter {
				disableAWGAfterCommit = true
			}
			push = false
		}
		...
		if isLocalAWGAfter && oldInbound.Enable {
